...
class RunStore:
    ...
    def prompt_path(self, run_id, attempt):
        return self.run_dir(run_id) / "prompts" / f"attempt_{int(attempt):03d}.txt"
    ...
    def write_prompt(self, task_state, attempt, prompt):
        path = self.prompt_path(task_state, attempt)
        path.parent.mkdir(parents=True, exist_ok=True)
        self._write_text_atomic(path, str(prompt))
        return path
