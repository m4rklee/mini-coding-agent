...
class DurableMemoryStore:
    ...
    '''
        三级排序方法：
        1.tag精确匹配：用户请求的分词结果中包含长期记忆的tag
        2.关键词重叠：用户请求的分词结果与记忆笔记内容、记忆主题和tag有重叠
        3.创建时间：越新越好
    '''
...
def normalize_memory_state(state, workspace_root=None):
    ...
    durable_root = Path(workspace_root) / ".mca" / "memory" if workspace_root is not None else None
...
def retrieval_candidates(state, query, limit=3, workspace_root=None):
    ...
    if workspace_root is not None:
        durable_store = DurableMemoryStore(Path(workspace_root) / ".mca" / "memory")
...
class LayeredMemory:
    def __init__(self, state=None, workspace_root=None):
        ...
        self.durable_store = DurableMemoryStore(Path(workspace_root) / ".mca" / "memory") if workspace_root is not None else None
