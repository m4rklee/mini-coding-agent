...
Mca 就是包在模型外面的控制循环：负责组 prompt、解析模型输出、
...
class Mca:
    def __init__(
        ...
        self.run_store = run_store or RunStore(Path(workspace.repo_root) / ".mca" / "runs")
    ...
    def close(self):
        for client in list(getattr(self, "mcp_clients", {}).values()):
            try:
                client.close_sync()
            except Exception:
                pass
    ...
    '''
        ...
        memory：记忆
        checkpoint：支持断点续传
        运行时身份：模型名、工作区指纹、工具签名等
        恢复状态：记录checkpoint是否有效、哪些文件已经过期等
    '''
    ...
    # 清理过期的文件记忆，防止Agent根据旧信息做错误决策
    # 会话恢复后，项目原有的文件可能已经被修改了，需要Agent重新读取
    def invalidate_stale_memory(self):
        invalidated = self.memory.invalidate_stale_file_summaries()
        self.session["memory"] = self.memory.to_dict()
        return invalidated

    # 恢复后评估保存的状态是否仍然可靠，解决漂移问题
    '''
        Schema 版本：Checkpoint的数据格式版本是否与当前代码兼容？
        key_files：checkpoint记录的关键文件，在磁盘上是否被改过了？
        runtime_identity：运行时身份（模型、工作区、配置）是否与保存时一致？
    '''
    ...
    def build_skills(self):
        return toolkit.build_skill_registry(self)
    
    # 生成工具配置的“指纹”，用于检测Agent的可用工具是否发生变化，用于缓存判断和恢复后的一致性检查
    ...
    def build_prefix(self):
        ...
        if mode == "plan":
            mode_rules = textwrap.dedent(
                ...
                Plan mode rules:
                - Do not modify files.
                - Do not call risky tools such as write_file, patch_file, run_shell, or mutating MCP tools.
                - You may call read-only tools to inspect the workspace.
                - Prefer returning a concise plan over taking action.
            ...
            mode_rules = textwrap.dedent(
                ...
                ReAct mode rules:
                - Use tools as needed to inspect, edit, and verify the workspace.
                - When the user asks for implementation, proceed without stopping at a plan.
                - After changes, verify with focused tests or commands when practical.
                """
            ).strip()
        ...
        text = textwrap.dedent(
            f"""\
            You are mca, a small local coding agent working inside a local repository.
    ...
    # prompt和当前会话的元数据一并生成，元数据用于追踪运行状态
    ...
    def emit_runtime_event(self, event, payload=None):
        if not self.event_sink:
            return
        try:
            self.event_sink(event, self.redact_artifact(payload or {}))
        except Exception:
            return

    # 捕获工作区快照
    # 在执行危险操作（写文件、patch、shell命令）前后，记录整个工作区所有文件的状态
    ...
    # 创建检查点
    # 在 Agent 运行的关键节点，保存“当前现场”，方便后续恢复
    # 用于任务机制中的任务保存和恢复
    ...
    # 对于工具执行结果，并不全部组装到prompt中
    # 先将工具执行结果进入完整历史，由这个函数择优沉淀为上下文记忆，放入prompt
    ...
    # 从模型回答中提取值得长期保存的知识
    '''
        用户消息检查：使用规则匹配，查看是否有需要记忆的事件
        模型消息检查：逐行扫描模型回复，用规则匹配的方式找到需要记忆的事件
    '''
    ...
    '''
        promoted: 需要入库的记忆
        rejections：过滤掉的不入库的记忆
        superseded：已有记忆中存在相同主语的记忆，需要用新条目替换
    '''
    ...
    def _render_tool_call(self, name, args):
        ...
        if name == "read_file":
            ...
            desc = f"List files in {args.get('path', '.')}"
        elif name == "run_shell":
            cmd = args.get('command', '')[:40]
            desc = f"Run shell: {cmd}"
        elif name == "search":
            desc = f"Search for '{args.get('query', '')}'"
        elif name == "write_file":
            desc = f"Write {args.get('path', '')}"
        elif name == "patch_file":
            desc = f"Patch {args.get('path', '')}"
        elif name == "delegate":
            desc = f"Delegate: {args.get('task', '')[:40]}"
        else:
            desc = f"Tool: {name}"
        print(f"\n  \033[36m●\033[0m {desc}")

    # 完整的agent流程
    def ask(self, user_message, stream=False, on_chunk=None):
        ...
        如果新人想理解 mca 是怎么“从一句话跑成一个 agent 流程”的，
        这里就是最关键的入口。
        """
        ...
        while tool_steps < self.max_steps and attempts < max_attempts:
            ...
            prompt_path = self.run_store.write_prompt(task_state, task_state.attempts, prompt)
            prompt_relative_path = prompt_path.relative_to(self.run_store.run_dir(task_state)).as_posix()
            self.emit_trace(
                task_state,
                "prompt_built",
                {
            ...
            # 三种更新checkpoint的情况
            '''
                1.文件过期：旧文件被修改，导致哈希得到的新鲜度与不同时，需要根据目前情况重新创建checkpoint
                2.环境变化：工作目录环境变化时，需要重新创建checkpoint
                3.上下文压缩：当上下文压缩后，需要重新创建checkpoint
            '''
            ...
            if stream:
                ...
                if on_chunk and hasattr(on_chunk, "reset"):
                    on_chunk.reset()
                for chunk in self.model_client.complete_stream(
                    prompt,
                    self.max_new_tokens,
                    ...
                    buffer += chunk
                    if output_state == "unknown":
                        preview = buffer.lstrip()[:50]
                        if preview.startswith("<tool"):
                            output_state = "tool"
                        else:
                            output_state = "final"
                    if on_chunk:
                        on_chunk(chunk, output_state)
                raw = buffer
            else:
            ...
            if kind == "tool":
                ...
                self.emit_runtime_event("tool_started", {"name": name, "args": args})
            ...
            if stream:
                if on_chunk and getattr(on_chunk, "printed", 0) == 0:
                    return final
                print()
                return ""
            return final
    ...
    def tool_load_skill(self, args):
        return toolkit.tool_load_skill(self, args)
    ...
    def parse(raw):
        ...
        if "<tool>" in raw and ("<final>" not in raw or raw.find("<tool>") < raw.find("<final>")):
            body = Mca.extract(raw, "tool")
            try:
                payload = json.loads(body)
            except Exception:
                return "retry", Mca.retry_notice("model returned malformed tool JSON")
            if not isinstance(payload, dict):
                return "retry", Mca.retry_notice("tool payload must be a JSON object")
            if not str(payload.get("name", "")).strip():
                return "retry", Mca.retry_notice("tool payload is missing a tool name")
            args = payload.get("args", {})
            if args is None:
                payload["args"] = {}
            elif not isinstance(args, dict):
                return "retry", Mca.retry_notice()
            return "tool", payload
        if "<tool" in raw and ("<final>" not in raw or raw.find("<tool") < raw.find("<final>")):
            payload = Mca.parse_xml_tool(raw)
            if payload is not None:
                return "tool", payload
            return "retry", Mca.retry_notice()
        if "<final>" in raw:
            final = Mca.extract(raw, "final").strip()
            if final:
                return "final", final
            return "retry", Mca.retry_notice("model returned an empty <final> answer")
        raw = raw.strip()
        if raw:
            return "final", raw
        return "retry", Mca.retry_notice("model returned an empty response")
    ...
    def parse_xml_tool(raw):
        ...
        if not match:
            return None
        attrs = Mca.parse_attrs(match.group("attrs"))
        ...
        for key in ("content", "old_text", "new_text", "command", "task", "pattern", "path"):
            if f"<{key}>" in body:
                args[key] = Mca.extract_raw(body, key)
