...
from mca.run_store import RunStore
from mca.task_state import STOP_REASON_FINAL_ANSWER_RETURNED, TaskState


def test_run_store_creates_run_directory_and_state_file(tmp_path):
    store = RunStore(tmp_path / ".mca" / "runs")
...
def test_run_store_writes_prompt_attempt_file(tmp_path):
    store = RunStore(tmp_path / ".mca" / "runs")
    state = TaskState.create(run_id="run_prompt", task_id="task_prompt", user_request="Trace prompts.")
    store.start_run(state)

    path = store.write_prompt(state, 2, "full prompt\nwith transcript")

    assert path == store.run_dir(state.run_id) / "prompts" / "attempt_002.txt"
    assert path.read_text(encoding="utf-8") == "full prompt\nwith transcript"
