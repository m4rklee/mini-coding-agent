...
from mca.cli import StreamPrinter, _handle_model_command, _handle_slash_command
...
def test_agent_saves_full_prompt_for_each_model_attempt(tmp_path):
    agent = build_agent(
        tmp_path,
        [
            "",
    ...
    assert agent.ask("Inspect prompt saving") == "Recovered."

    run_dir = agent.run_store.run_dir(agent.current_task_state)
    first_prompt = run_dir / "prompts" / "attempt_001.txt"
    second_prompt = run_dir / "prompts" / "attempt_002.txt"
    assert first_prompt.exists()
    assert second_prompt.exists()
    assert "Current user request:\nInspect prompt saving" in first_prompt.read_text(encoding="utf-8")
    assert "Runtime notice: model returned an empty response" in second_prompt.read_text(encoding="utf-8")

    trace_events = [
        json.loads(line)
        for line in agent.run_store.trace_path(agent.current_task_state).read_text(encoding="utf-8").splitlines()
    ]
    prompt_events = [event for event in trace_events if event.get("event") == "prompt_built"]
    assert [event["prompt_path"] for event in prompt_events] == [
        "prompts/attempt_001.txt",
        "prompts/attempt_002.txt",
    ]
...
def test_list_files_hides_internal_agent_state(tmp_path):
    agent = build_agent(tmp_path, [])
    (tmp_path / ".mca").mkdir(exist_ok=True)
    (tmp_path / ".git").mkdir(exist_ok=True)
    (tmp_path / "hello.txt").write_text("hi\n", encoding="utf-8")

    result = agent.run_tool("list_files", {})

    assert ".mca" not in result
...
def test_openai_compatible_client_posts_expected_responses_payload():
    ...
    assert captured["headers"]["User-agent"] == "mca/0.1"
...
def test_openai_compatible_client_stream_ignores_done_text_after_deltas():
    class FakeResponse:
        def __enter__(self):
            return self
        ...
        def __iter__(self):
            lines = (
                'data: {"type":"response.output_text.delta","delta":"<final>"}\n',
                'data: {"type":"response.output_text.delta","delta":"OK</final>"}\n',
                'data: {"type":"response.output_text.done","text":"<final>OK</final>"}\n',
                "data: [DONE]\n",
            )
    ...
    with patch("urllib.request.urlopen", return_value=FakeResponse()):
        result = "".join(client.complete_stream("hello", 42))

    assert result == "<final>OK</final>"


def test_openai_compatible_client_stream_uses_done_text_without_deltas():
    class FakeResponse:
        def __enter__(self):
            return self
...
def test_stream_printer_hides_split_final_tags():
    output = []
    printer = StreamPrinter(output.append)

    for chunk in ("<fi", "nal>OK</fi", "nal>"):
        printer(chunk, "final")

    assert "".join(output) == "OK"


def test_stream_printer_hides_repeated_final_tag_fragments():
    output = []
    printer = StreamPrinter(output.append)

    for chunk in ("<final>OK", "<fi", "nal>OK</final>"):
        printer(chunk, "final")

    assert "".join(output) == "OK"


def test_stream_printer_hides_tool_chunks():
    output = []
    printer = StreamPrinter(output.append)

    printer('<tool name="read_file" path="README.md"></tool>', "tool")

    assert output == []


def test_stream_printer_hides_prose_before_tool_tag():
    output = []
    printer = StreamPrinter(output.append)

    printer("I will inspect first.\n", "final")
    printer('<tool>{"name":"list_files","args":{"path":"."}}</tool>', "final")

    assert output == []


def test_stream_printer_waits_for_final_tag_before_printing():
    output = []
    printer = StreamPrinter(output.append)

    printer("Plain final without tag", "final")
...
def test_build_agent_uses_openai_provider_and_model_override(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            "mca.cli.OllamaModelClient",
            side_effect=AssertionError("ollama client should not be used"),
        ), patch("mca.cli.OpenAICompatibleModelClient") as mock_openai:
...
def test_model_command_show_prints_current_model(tmp_path, capsys):
    args = mini_pkg.build_arg_parser().parse_args(
        ["--cwd", str(tmp_path), "--provider", "openai", "--model", "start-model"]
    )

    with patch("mca.cli.OpenAICompatibleModelClient") as mock_openai:
        fake_client = mock_openai.return_value
        fake_client.model = "start-model"
        fake_client.base_url = "https://www.right.codes/codex/v1"
        agent = mini_pkg.build_agent(args)

    _handle_model_command(agent, "/model show")

    output = capsys.readouterr().out
    assert "provider: openai" in output
    assert "model: start-model" in output
    assert "client:" in output
    assert "endpoint: https://www.right.codes/codex/v1" in output


def test_model_command_sets_provider_and_model(tmp_path, capsys):
    args = mini_pkg.build_arg_parser().parse_args(
        ["--cwd", str(tmp_path), "--provider", "openai", "--model", "start-model"]
    )

    with patch("mca.cli.OpenAICompatibleModelClient") as mock_openai, patch(
        "mca.cli.AnthropicCompatibleModelClient"
    ) as mock_anthropic:
        initial_client = mock_openai.return_value
        next_client = mock_anthropic.return_value
        agent = mini_pkg.build_agent(args)
        _handle_model_command(agent, "/model set provider=deepseek model=deepseek-v4-flash")

    mock_anthropic.assert_called_once()
    assert mock_anthropic.call_args.kwargs["model"] == "deepseek-v4-flash"
    assert agent.model_client is next_client
    assert agent.model_client is not initial_client
    assert agent.model_config["provider"] == "deepseek"
    assert agent.model_config["model"] == "deepseek-v4-flash"
    assert "model updated: provider=deepseek model=deepseek-v4-flash" in capsys.readouterr().out


def test_model_command_sets_provider_with_default_model(tmp_path):
    args = mini_pkg.build_arg_parser().parse_args(
        ["--cwd", str(tmp_path), "--provider", "openai", "--model", "start-model"]
    )

    with patch.dict(os.environ, {}, clear=True), patch("mca.cli.OpenAICompatibleModelClient"), patch(
        "mca.cli.AnthropicCompatibleModelClient"
    ) as mock_anthropic:
        agent = mini_pkg.build_agent(args)
        _handle_model_command(agent, "/model set provider=deepseek")
...
def test_model_command_sets_model_for_current_provider(tmp_path):
    args = mini_pkg.build_arg_parser().parse_args(
        ["--cwd", str(tmp_path), "--provider", "openai", "--model", "start-model"]
    )
    ...
    with patch("mca.cli.OpenAICompatibleModelClient") as mock_openai:
        agent = mini_pkg.build_agent(args)
        _handle_model_command(agent, "/model set model=custom-model")

    assert mock_openai.call_count == 2
    assert mock_openai.call_args.kwargs["model"] == "custom-model"
    assert agent.model_config["provider"] == "openai"
    assert agent.model_config["model"] == "custom-model"


def test_model_command_rejects_invalid_provider_without_replacing_client(tmp_path, capsys):
    args = mini_pkg.build_arg_parser().parse_args(
        ["--cwd", str(tmp_path), "--provider", "openai", "--model", "start-model"]
    )

    with patch("mca.cli.OpenAICompatibleModelClient"):
        agent = mini_pkg.build_agent(args)
    original_client = agent.model_client

    _handle_model_command(agent, "/model set provider=bogus")

    assert agent.model_client is original_client
    output = capsys.readouterr().out
    assert "unsupported provider: bogus" in output
    assert "Usage:" in output


def test_model_command_rejects_unknown_option_without_replacing_client(tmp_path, capsys):
    args = mini_pkg.build_arg_parser().parse_args(
        ["--cwd", str(tmp_path), "--provider", "openai", "--model", "start-model"]
    )
    ...
    _handle_model_command(agent, "/model set base-url=https://example.test/v1")

    assert agent.model_client is original_client
    output = capsys.readouterr().out
    assert "unknown model option: base-url" in output
    assert "Usage:" in output


def test_model_slash_command_is_handled_without_calling_agent_ask(tmp_path):
    args = mini_pkg.build_arg_parser().parse_args(
        ["--cwd", str(tmp_path), "--provider", "openai", "--model", "start-model"]
    )
    ...
    with patch("mca.cli.OpenAICompatibleModelClient"):
        agent = mini_pkg.build_agent(args)
    agent.ask = MagicMock(side_effect=AssertionError("agent.ask should not be called"))

    assert _handle_slash_command("/model show", agent) is True
    agent.ask.assert_not_called()


def test_skill_slash_command_prints_available_skills_without_calling_agent_ask(tmp_path, capsys):
    skills_dir = tmp_path / "skills" / "hello"
    ...
    assert _handle_slash_command("/skill", agent) is True
    agent.ask.assert_not_called()
    output = capsys.readouterr().out
    assert "Skills:" in output
    assert "hello" in output
    assert "Say hello when greeted." in output
...
def test_build_agent_uses_anthropic_provider_and_openai_key_fallback(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            ...
            "mca.cli.OpenAICompatibleModelClient",
            side_effect=AssertionError("openai client should not be used"),
        ), patch("mca.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_build_agent_uses_anthropic_default_model_when_env_is_missing(tmp_path):
    ...
    with patch.dict(
        ...
        with patch("mca.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_build_agent_uses_deepseek_provider_and_env_configuration(tmp_path):
    (tmp_path / ".env").write_text(
        "\n".join(
            [
                "MCA_DEEPSEEK_API_BASE=https://api.deepseek.com/anthropic",
                "MCA_DEEPSEEK_API_KEY=sk-project-deepseek",
...
def test_explicit_memory_promotion_persists_durable_memory_topics(tmp_path):
    agent = build_agent(
        ...
        [
            ...
            "Project convention: Preserve local agent state under .mca/.\n"
    ...
    index_path = tmp_path / ".mca" / "memory" / "MEMORY.md"
    conventions_path = tmp_path / ".mca" / "memory" / "topics" / "project-conventions.md"
    decisions_path = tmp_path / ".mca" / "memory" / "topics" / "key-decisions.md"
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    ...
    assert report["durable_promotions"] == [
        ...
        "project-conventions: Preserve local agent state under .mca/.",
...
def test_explicit_memory_promotion_rejects_secret_shaped_and_transient_lines(tmp_path):
    ...
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    conventions_path = tmp_path / ".mca" / "memory" / "topics" / "project-conventions.md"
    dependency_path = tmp_path / ".mca" / "memory" / "topics" / "dependency-facts.md"
...
def test_public_api_exports_resolve_through_package_path():
    ...
    assert Path(mini_pkg.__file__).as_posix().endswith("/mca/__init__.py")
